...
public class VaadinMovieListView extends UI implements MovieListView {
	...
	protected void init(VaadinRequest request) {
		...
		presenter = new MovieListPresenter(
				new SpringInjector().getBean(MovieRepository.class), this);
		presenter.onStart();
	}
	...
	private void initListeners() {
		add.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				Movie rowId = (Movie) movies.getValue(); // get the selected rows id
				System.out.println(rowId);
				presenter.addMovie(title.getValue(), directors.getValue(), year.getValue());
			}
		});
	...
	public void addMovieToList(Movie movie) {
		movies.addItem(new Object[] { movie.getTitle(), movie.getDirector(),
				movie.getYear() }, movie);
	}
