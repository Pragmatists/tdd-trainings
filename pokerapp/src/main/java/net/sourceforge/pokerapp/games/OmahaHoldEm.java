...
/***********************
 * Constructor - creates a new OmahaHoldEm game
 * This constructor is so Omaha Hold'Em can be extened with other game variations
 *
 * @param a The StartPoker instance to which this game belongs
 * @param gameName The name of the game to play
 * @param selfInit Whether this game is able to initialize itself
 *
 **/
...
//   Fourth card down
        ...
        theApp.broadcastMessage( "MESSAGE  &Playing Omaha Hold'Em.  " + ( (Player)theApp.getPlayerList().get(currPlayerIndex) ).getName() + "'s bet.");  
 ...
 **/
    protected void nextAction() {
        theApp.log( "OmahaHoldEm.nextAction() - previous actionNum = " + actionNum, 3 );
        actionNum++;
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
                ...
                for ( int a = 0; a < numShareCards-2; a++ ) { 
                    for ( int b = a+1; b < numShareCards-1; b++ ) {
                        for ( int c = b+1; c < numShareCards; c++ ) {
                            test = he.rankHand( chole[i],chole[j],cshare[a],cshare[b],cshare[c] );
                            if ( test > best ) { 
                                best = test;
                            }
                        }
