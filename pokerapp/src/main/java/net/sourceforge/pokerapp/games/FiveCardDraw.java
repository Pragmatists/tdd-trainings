...
public class FiveCardDraw extends PokerGame {
	...
	protected void deal() {
		...
		theApp.broadcastMessage("MESSAGE  &Playing Five Card Draw.  "
	...
	protected void nextAction() {
		theApp.log("FiveCardDraw.nextAction() - previous actionNum = "
				+ actionNum, 3);
		actionNum++;
		for (int i = 0; i < theApp.getPlayerList().size(); i++) {
	...
	/***********************
	 * draw() defines what happens when its time for players to draw their new
	 * cards.
	 **/
	private void draw() {
		...
		if (!allDrawn) {
			...
			if (!drawn[currPlayerIndex]) {
				theApp.broadcastMessage("MESSAGE &" + currPlayer.getName()
						+ " is deciding how many cards to draw...");
				theApp.messageToPlayer(currPlayer.getName(),
						"DISPLAY DRAW BUTTON");
				theApp.messageToPlayer(
						currPlayer.getName(),
						"MESSAGE &Please select cards to draw for and press Draw Cards button on table when ready.");
			} else {
				allDrawn = true;
				for (int i = 0; i < theApp.getPlayerList().size(); i++) {
	 ...
	 * @param b
	 *            Whether this player has to turn his cards face up.
	 * 
	 **/
	...
	protected void mouseClick(String name, int x, int y) {
		theApp.log("FiveCardDraw.mouseClick( " + name + ", " + x + ", " + y
				+ " )", 3);
		Player currPlayer = (Player) theApp.getPlayerList()
				.get(currPlayerIndex);
		...
		if ((name.equals(currPlayer.getName())) && (actionNum == 2)) {
			...
			// Player has clicked the "Draw Cards" button in the middle of the
			// screen.
			//
			if ((x >= 340) && (x <= 340 + 150) && (y >= 200 + 23)
					&& (y <= 200 + 23 + 50)) {
				theApp.log(
						name + " has selected to draw " + numSel + " cards.", 1);
				drawn[currPlayerIndex] = true;
				theApp.broadcastMessage("PLAYER CALL  &" + currPlayer.getName()
						+ "&Drew " + numSel + " cards&"
				...
				((Player) theApp.getPlayerList().get(currPlayerIndex))
						.getHand().clearHand();
				theApp.messageToPlayer(currPlayer.getName(),
						"PLAYER CLEAR HAND &" + currPlayer.getName());
				for (int i = 0; i < 5; i++) {
					Card c = new Card();
					if (drawSelections[i]) {
						c = theApp.getDeck().deal();
					...
					theApp.messageToPlayer(currPlayer.getName(), "CARD HOLE  &"
							+ currPlayer.getName() + "&" + i + "&" + c);
			...
			for (int cardNum = 0; cardNum < 5; cardNum++) {
				...
				if ((x >= PokerApp.LOCATION[loc][0] + 30 * cardNum)
					...
					if (drawSelections[cardNum]) {
						theApp.messageToPlayer(currPlayer.getName(),
								"DESELECT CARD &" + cardNum);
						drawSelections[cardNum] = !drawSelections[cardNum];
					} else if ((numSel < 3)
							|| (ace
									&& (numSel < 4)
									&& (currPlayer.getHand()
											.getHoleCard(cardNum).getRank() != Card.ACE) && (!aceSel))) {
						theApp.messageToPlayer(currPlayer.getName(),
								"SELECT CARD &" + cardNum);
						drawSelections[cardNum] = !drawSelections[cardNum];
					}
