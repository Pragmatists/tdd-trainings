 ...
 **/
    protected void nextAction() {
        theApp.log( "HideTheSalami.nextAction() - previous actionNum = " + actionNum, 3 );
        actionNum++;
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
            ...
            for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
                ...
                if ( p.in ) {
                    ...
                    for ( int c = 0; c < newHand.getNumHole(); c++ ) {
                        ((Player)theApp.getPlayerList().get(i)).getHand().addHoleCard( newHand.getHoleCard(c) );
                        theApp.messageToPlayer( p.getName(), "CARD HOLE  &" + p.getName() + "&" + c + "&" + newHand.getHoleCard(c) );
                        theApp.broadcastMessage( "CARD HOLE  &" + p.getName() + "&" + c );
                    }
                    for ( int c = 0; c < newHand.getNumUp(); c++ ) {
                        ((Player)theApp.getPlayerList().get(i)).getHand().addUpCard( newHand.getUpCard(c) );
                        theApp.broadcastMessage( "CARD UP  &" + p.getName() + "&" + (c+newHand.getNumHole()) + "&" + newHand.getUpCard(c) );
                    }
                }
            }
            nextAction();
 ...
 **/
    protected void mouseClick( String name, int x, int y ) {
        theApp.log( "HideTheSalami.mouseClick( " + name + ", " + x + ", " + y + " )", 3 );
...
//
            if ( ( x >= 340 ) && ( x <= 340+150 ) && ( y >= 200+23 ) && ( y <= 200+23+50 ) ) {
                if ( numSel == 3 ) {
                    ...
                    theApp.messageToPlayer( name, "MESSAGE &You must select 3 cards to flip." );
                    return;
                }
            }
//
