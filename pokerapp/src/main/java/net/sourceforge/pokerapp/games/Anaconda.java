...
public class Anaconda extends PokerGame {
    ...
    private boolean  allPassed;             // Has everybody had a chance to pass their cards?
...
//   Sixth card down
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
            c = theApp.getDeck().deal();
            ((Player)theApp.getPlayerList().get(i) ).getHand().addHoleCard( c );
            theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(i) ).getName(), "CARD HOLE  &" + ( (Player)theApp.getPlayerList().get(i) ).getName() + "&5&" + c );
            theApp.broadcastMessage( "CARD HOLE  &" + ( (Player)theApp.getPlayerList().get(i) ).getName() + "&5" );
        }
...
//   Seventh card down
        ...
        theApp.broadcastMessage( "MESSAGE  &Playing Anaconda.  " + ( (Player)theApp.getPlayerList().get(currPlayerIndex) ).getName() + "'s bet.");  
 ...
 **/
    protected void nextAction() {
        theApp.log( "Anaconda.nextAction() - previous actionNum = " + actionNum, 3 );
        actionNum++;
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
...
/***********************
 * pass() defines what happens when its time for players to pass their cards to the left
 *
 * @param num The number of cards that will be passed this round.
 *
 **/
    private void pass( int num ) {
        ...
        if ( !allPassed ) {
            ...
            for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
                if ( ( ((Player)theApp.getPlayerList().get(i)).in ) && ( !passed[i] ) ) {
                    theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(i)).getName(), "DISPLAY PASS BUTTON" ); 
                }
            }
        } else {
            allPassed = false;
            ...
            while ( !done ) {
                ...
                if ( ( ((Player)theApp.getPlayerList().get(pindex)).in ) && ( ((Player)theApp.getPlayerList().get(pindex)).getHand().getNumHole() >= 7 ) ) {
                    Hand tempHand = new Hand();
                    Hand passHand = new Hand();
                    for ( int cindex = 0; cindex < 7; cindex++ ) {
                        if ( passSelections[pindex][cindex] ) {
                            passHand.addHoleCard( ((Player)theApp.getPlayerList().get(pindex)).getHand().getHoleCard(cindex) );
                        } else {
                            tempHand.addHoleCard( ((Player)theApp.getPlayerList().get(pindex)).getHand().getHoleCard(cindex) );
                        }
                    }
                    ((Player)theApp.getPlayerList().get(pindex)).getHand().clearHand();
                    theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(pindex)).getName(), "PLAYER CLEAR HAND &" + ((Player)theApp.getPlayerList().get(pindex)).getName() );
                    int cindex = 0;
                    for ( int tempindex = 0; tempindex < tempHand.getNumHole(); tempindex++ ) {
                        Card c = tempHand.getHoleCard(tempindex);
                        ((Player)theApp.getPlayerList().get(pindex) ).getHand().addHoleCard( c );
                        theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(pindex) ).getName(), "CARD HOLE  &" + ((Player)theApp.getPlayerList().get(pindex)).getName() + "&" + cindex + "&" + c );
                        cindex++;
                    }
                    for ( int passindex = 0; passindex < prevPassHand.getNumHole(); passindex++ ) {
                    ...
                    for ( int passindex = 0; passindex < passHand.getNumHole(); passindex++ ) {
                        prevPassHand.addHoleCard( passHand.getHoleCard(passindex) );
                    }
                }
                ...
                if ( sindex == firstSeat ) {
                    ...
                    int cindex = ((Player)theApp.getPlayerList().get(pindex) ).getHand().getNumHole();
                    for ( int passindex = 0; passindex < prevPassHand.getNumHole(); passindex++ ) {
                        Card c = prevPassHand.getHoleCard(passindex);
                        ((Player)theApp.getPlayerList().get(pindex) ).getHand().addHoleCard( c );
                        theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(pindex) ).getName(), "CARD HOLE  &" + ((Player)theApp.getPlayerList().get(pindex)).getName() + "&" + cindex + "&" + c );
...
/***********************
 * flipCard() defines what happens when its time for players to start flipping over their cards.
 *
 * @param num The card number that gets flipped over
 *
 **/
        ...
        for ( int pindex = 0; pindex < theApp.getPlayerList().size(); pindex++ ) {
            ...
            if ( player.in ) {
                ...
                for ( int tempindex = 0; tempindex < tempHand.getNumHole(); tempindex++ ) {
                    Card c = tempHand.getHoleCard(tempindex);
                    if ( tempindex <= num ) {
                        ((Player)theApp.getPlayerList().get(pindex) ).getHand().addUpCard( c );
                        theApp.broadcastMessage( "CARD UP  &" + ( (Player)theApp.getPlayerList().get(pindex) ).getName() + "&" + tempindex + "&" + c );
                    } else {
                        ((Player)theApp.getPlayerList().get(pindex) ).getHand().addHoleCard( c );
                        theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(pindex) ).getName(), "CARD HOLE  &" + ((Player)theApp.getPlayerList().get(pindex)).getName() + "&" + tempindex + "&" + c );
                        theApp.broadcastMessage( "CARD HOLE  &" + ( (Player)theApp.getPlayerList().get(pindex) ).getName() + "&" + tempindex );
                    }
                }
            }
        }
...
/***********************
 * pickCards() defines what happens when its time for players to pick their 5 best cards.
 **/
    private void pickCards() {
        ...
        if ( !allPassed ) {
            ...
            for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
                if ( ( ((Player)theApp.getPlayerList().get(i)).in ) && ( !passed[i] ) ) {
                    theApp.messageToPlayer( ((Player)theApp.getPlayerList().get(i)).getName(), "DISPLAY DONE BUTTON" ); 
                }         
            }
        } else {
            allPassed = false;
            ...
            for ( int pindex = 0; pindex < theApp.getPlayerList().size(); pindex++ ) {
                theApp.broadcastMessage( "PLAYER NOPICS &" + ((Player)theApp.getPlayerList().get(pindex)).getName() );
            }
            for ( int pindex = 0; pindex < theApp.getPlayerList().size(); pindex++ ) {
                if ( ( ((Player)theApp.getPlayerList().get(pindex)).in ) && ( ((Player)theApp.getPlayerList().get(pindex)).getHand().getNumHole() >= 5 ) ) {
                    Hand tempHand = new Hand();
 ...
 *
 * @param p The player being shown
 * @param b Whether or not this player is actually going to turn over their cards.
 *
 **/
    protected void showPlayer( Player p, boolean b ) {
        if ( b ) {
            theApp.broadcastMessage( "SHOW CARD &" + p.getName() + "&4&" + p.getHand().getHoleCard(0) );
        } else {
 ...
 * This function is mainly to aid the computer AI logic.
 *
 * @return The float value of the best possible hand based on what is showing
 *
 **/
 ...
 **/
    protected void mouseClick( String name, int x, int y ) {
        theApp.log( "Anaconda.mouseClick( " + name + ", " + x + ", " + y + " )", 3 );
...
//        Player has clicked the "Pass Cards" button in the middle of the screen.
//
            if ( ( x >= 340 ) && ( x <= 340+150 ) && ( y >= 200+23 ) && ( y <= 200+23+50 ) ) {
                if ( numSel == numToPass ) {
                    theApp.log( name + " has selected " + numToPass + " cards to pass.", 1 );
                    theApp.messageToPlayer( name, "MESSAGE &Waiting for everyone else to select cards to pass." );
                    passed[pindex] = true;
                    pass(numSel);
                    return;
                } else {
                    theApp.messageToPlayer( name, "MESSAGE &You must select " + numToPass + " cards to pass." );
                    return;
                }
            }
            ...
            for ( int cardNum = 0; cardNum < 7; cardNum++ ) {
                ...
                if ( ( x >= PokerApp.LOCATION[loc][0]+30*cardNum ) && ( x <= PokerApp.LOCATION[loc][0]+30*cardNum+width ) &&
                    ...
                    if ( passSelections[pindex][cardNum] ) {
                        theApp.messageToPlayer( name, "DESELECT CARD &" + cardNum );
...
//
            if ( ( x >= 340 ) && ( x <= 340+150 ) && ( y >= 200+23 ) && ( y <= 200+23+50 ) ) {
                ...
                theApp.messageToPlayer( name, "MESSAGE &Waiting for everyone else to reorder their cards." );
                passed[pindex] = true;
                pickCards();
                return;
            }
            ...
            for ( int cardNum = 0; cardNum < 7; cardNum++ ) {
                ...
                if ( ( x >= PokerApp.LOCATION[loc][0]+30*cardNum ) && ( x <= PokerApp.LOCATION[loc][0]+30*cardNum+width ) &&
                    ...
                    if ( passSelections[pindex][cardNum] ) {
                        ...
                        Card c1 = ((Player)theApp.getPlayerList().get(pindex)).getHand().getHoleCard( cardNum );
                        Card c2 = ((Player)theApp.getPlayerList().get(pindex)).getHand().getHoleCard( c2Num );
                        Hand tempHand = new Hand();
                        for ( int cindex = 0; cindex < ((Player)theApp.getPlayerList().get(pindex)).getHand().getNumHole(); cindex++ ) {
                            if ( cindex == cardNum ) {
