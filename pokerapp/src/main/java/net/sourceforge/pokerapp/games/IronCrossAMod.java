...
//   The AMod means any single shared card can change rank up or down by 1 or change to any suit
...
//    Check all suit changes first
                for ( int a = 1; a < 5; a++ ) {
                    ...
                    test = he.rankHand( chole[i],chole[j],changed,cshare[2],cshare[4] );
                    if ( test > best ) {
                        best = test;
                    }

                    changed = new Card( cshare[2].getRank(), a );
                    test = he.rankHand( chole[i],chole[j],cshare[0],changed,cshare[4] );
                    if ( test > best ) {
                        best = test;
                    }

                    changed = new Card( cshare[4].getRank(), a );
                    test = he.rankHand( chole[i],chole[j],cshare[0],cshare[2],changed );
                    if ( test > best ) {
                        best = test;
                    }

                    changed = new Card( cshare[1].getRank(), a );
                    test = he.rankHand( chole[i],chole[j],changed,cshare[3],cshare[4] );
                    if ( test > best ) {
                        best = test;
                    }

                    changed = new Card( cshare[3].getRank(), a );
                    test = he.rankHand( chole[i],chole[j],cshare[1],changed,cshare[4] );
                    if ( test > best ) {
                        best = test;
                    }

                    changed = new Card( cshare[4].getRank(), a );
                    test = he.rankHand( chole[i],chole[j],cshare[1],cshare[3],changed );
                    if ( test > best ) {
                        best = test;
                    }
                }
