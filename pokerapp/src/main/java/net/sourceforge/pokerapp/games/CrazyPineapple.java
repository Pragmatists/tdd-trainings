...
/****************************************************
 * Crazy Pineapple game definition.  A Texas Hold'em variant
 *
 * @author Dan Puperi
 * @version 1.00
 *
 **/
public class CrazyPineapple extends TexasHoldEm {
...
/***********************
 * Constructor - creates a new CrazyPineapple game
 *
 * @param a The StartPoker instance to which this game belongs
 *
 **/
    public CrazyPineapple( StartPoker a ) {
        super( a, "Crazy Pineapple", false );
        maxActionNum = 6;
        if ( theApp.getPlayerList().size() < 2 ) {
            return;
        }
        theApp.log( "Construction a game of CrazyPineapple", 3 );
 ...
 **/
    protected void deal() {
        theApp.log( "CrazyPineapple.deal()", 3 );
        Card  c = new Card();    
...
//   Third card down
        ...
        theApp.broadcastMessage( "MESSAGE  &Playing Crazy Pineapple.  " + ( (Player)theApp.getPlayerList().get(currPlayerIndex) ).getName() + "'s bet.");
 ...
 **/
    protected void nextAction() {
        theApp.log( "CrazyPineapple.nextAction() - previous actionNum = " + actionNum, 3 );
        actionNum++;
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
...
/***********************
 * dropCard() defines what happens when its time for all players to discard their cards.
 **/
    protected void dropCard() {
        theApp.log( "CrazyPineapple.dropCard()", 3 );
        allDiscarded = true;
        for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
            if ( ( ((Player)theApp.getPlayerList().get(i)).in ) && ( !discarded[i] ) ) {
                allDiscarded = false;
            }
        }
            ...
            for ( int i = 0; i < theApp.getPlayerList().size(); i++ ) {
                ...
                if ( p.in ) {
                    ...
                    if ( ( cardToDrop[i] >= 0 ) && ( cardToDrop[i] < numCards ) ) {
                        int numShare = p.getHand().getNumHole();
                        Card shareCards[] = new Card[numShare];
                        for ( int c = 0; c < numShare; c++ ) {
                            shareCards[c] = ((Player)theApp.getPlayerList().get(i)).getHand().getSharedCard(c);
                        }
                        Card holeCards[] = new Card[numCards-1];
                        int cnum = 0;
                        for ( int c = 0; c < numCards; c++ ) {
                            if ( c != cardToDrop[i] ) {
                                holeCards[cnum] = ((Player)theApp.getPlayerList().get(i)).getHand().getHoleCard(c);
                                cnum++;
                            }
                        }
                        ((Player)theApp.getPlayerList().get(i)).getHand().clearHand();
                        theApp.messageToPlayer( p.getName(), "PLAYER CLEAR HAND &" + p.getName() );
                        for ( int c = 0; c < holeCards.length; c++ ) {
                            ((Player)theApp.getPlayerList().get(i)).getHand().addHoleCard( holeCards[c] );
                            theApp.messageToPlayer( p.getName(), "CARD HOLE  &" + p.getName() + "&" + c + "&" + holeCards[c] );
                            theApp.broadcastMessage( "CARD HOLE  &" + p.getName() + "&" + c );
                        }
                        for ( int c = 0; c < numShare; c++ ) {
                            ((Player)theApp.getPlayerList().get(i)).getHand().addSharedCard( shareCards[c] );
                            theApp.messageToPlayer( p.getName(), "CARD SHARED  &" + p.getName() + "&" + c + "&" + shareCards[c] );
                        }
                    } 
                }
            }
 ...
 **/
    protected void mouseClick( String name, int x, int y ) {
        theApp.log( "CrazyPineapple.mouseClick( " + name + ", " + x + ", " + y + " )", 3 );
...
//
            if ( ( x >= 340 ) && ( x <= 340+150 ) && ( y >= 200+23 ) && ( y <= 200+23+50 ) ) {
                if ( cardToDrop[pindex] >= 0 ) {
                    theApp.log( name + " has selected which card to drop.", 1 );
                    theApp.messageToPlayer( name, "MESSAGE &Waiting for everyone else to select a card to drop." );
                    discarded[pindex] = true;
                    dropCard();
                    return;
                } else {
                    theApp.messageToPlayer( name, "MESSAGE &You must select a card to drop." );
                }
            }
            ...
            for ( int cardNum = 0; cardNum < 3; cardNum++ ) {
                ...
                if ( ( x >= PokerApp.LOCATION[loc][0]+30*cardNum ) && ( x <= PokerApp.LOCATION[loc][0]+30*cardNum+width ) &&
                    ...
                    if ( cardToDrop[pindex] < 0 ) {
                        ...
                        theApp.messageToPlayer( name, "DESELECT CARD &" + cardToDrop[pindex] );
                        theApp.messageToPlayer( name, "SELECT CARD &" + cardNum );
                        cardToDrop[pindex] = cardNum;
                    }
                }
