...
public class CrazyPineapple extends TexasHoldEm {
	...
	protected void deal() {
		...
		theApp.broadcastMessage("MESSAGE  &Playing Crazy Pineapple.  "
	...
	protected void nextAction() {
		theApp.log("CrazyPineapple.nextAction() - previous actionNum = "
				+ actionNum, 3);
		actionNum++;
		for (int i = 0; i < theApp.getPlayerList().size(); i++) {
				...
				if (p.in) {
					...
					if ((cardToDrop[i] >= 0) && (cardToDrop[i] < numCards)) {
						...
						for (int c = 0; c < numShare; c++) {
							shareCards[c] = ((Player) theApp.getPlayerList()
									.get(i)).getHand().getSharedCard(c);
						}
						...
						for (int c = 0; c < numShare; c++) {
							((Player) theApp.getPlayerList().get(i)).getHand()
									.addSharedCard(shareCards[c]);
							theApp.messageToPlayer(p.getName(),
									"CARD SHARED  &" + p.getName() + "&" + c
											+ "&" + shareCards[c]);
						}
					}
				}
	...
	protected void mouseClick(String name, int x, int y) {
		theApp.log("CrazyPineapple.mouseClick( " + name + ", " + x + ", " + y
				+ " )", 3);
