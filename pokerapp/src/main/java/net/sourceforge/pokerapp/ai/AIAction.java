...
/****************************************************
 * The AIAction class provides a way for the AI to interact with the game.  It receives
 * commands from the AILogic class and executes them.  It also receives data from the
 * poker game and passes that information on to the AI.
 *
 * @author Dan Puperi
 * @version 1.00
 *
...
public class AIAction {
    ...
    private        AILogic    ai;                              // The AILogic class that will decide what the AI should do

/***********************
 * Constructor creates a new AIAction instance
 *
 * @param a The AIApp instance to which this AIAction class belongs
 * @param type The type of AIAApp this is
 *
 **/
    public AIAction( AIApp a, String type ) {
        ...
        try {
            Class        ai_class    = Class.forName( "net.sourceforge.pokerapp.ai." + theAIApp.getAIClasses().get( theAIApp.getAILabels().indexOf( type ) ) );
...
/***********************
 * messageToAI() function checks commands from the server to the AI and acts on them.
 *
 * @param m The message sent to this AI
 *
 **/
...
//    Check to see if the refresh command is given - this will be a clue that it may be this AI's turn to deal
//
        if ( m.indexOf( "AI REFRESH" ) != -1 ) {
            int pIndex = theAIApp.playerIndex( theAIApp.getThisPlayer().getName() );
...
/***********************
 * deal() function is used when its the AI's turn to deal.  The AI will choose to deal a game
 * based on the gamePreferences() list which is set in AILogic.
 **/  
    private void deal() {
        ...
        try {
            ...
            if ( theAIApp.startUpApp.getGame() == null ) {
                ...
                for ( int k=0; k < ai.getGamePreference().size(); k++ ) {
                    total += ((Integer)ai.getGamePreference().get(k)).intValue();
                }
                ...
                for ( int i=0; i < ai.getGamePreference().size(); i++ ) {
                    int sum = 0;
                    for ( int j=0; j<=i; j++ ) { 
                        sum += ((Integer)ai.getGamePreference().get(j)).intValue();
                    }
                    if ( ( r <= sum ) && ( !gameDealt ) ) {
                        String gameName = (String)theAIApp.getGameLabels().get(i);
 ...
 *
 * @param actionList The list of possible actions this AI will take
 * @param probList The probability that the AI will choose the corresponding action from actionList
 ...
 **/
    public String action(ArrayList actionList, ArrayList probList ) {
        ...
        try {
            ...
            int pindex = theAIApp.startUpApp.playerIndex( theAIApp.getThisPlayer().getName() );
            int total = 0;
            ...
            for ( int i=0; i < probList.size(); i++) {
                ...
                if ( ( r <= sum ) && ( !actionPerformed ) ) {
                    ...
                    if ( ((String)actionList.get(i)).equals("CHECK") ) {
                        if ( theAIApp.startUpApp.getGame() != null ) {
                            if ( ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getPrevBet().amount() < theAIApp.startUpApp.getGame().getCurrBet().amount() ) {
                                theAIApp.messageToServer("BET FOLD");
                                theAIApp.log( "Wanted to check, but had to fold.", 3 );
                                return new String( "FOLD" );
                            } else {
                                theAIApp.messageToServer("BET CALL");
                                if ( theAIApp.startUpApp.getGame().getCurrBet().amount() == 0.0f ) {
                    ...
                    if ( ((String)actionList.get(i)).startsWith( new String("BET") ) ) {
                        if ( !theAIApp.noLimit && !theAIApp.betLimit && !theAIApp.potLimit ) {
                            theAIApp.messageToServer( "BET FOUR" );
                            ret = new String( "BET" );
                            theAIApp.log( "Bet", 3 );
                        } else {
                            String amounts[] = ( (String)actionList.get(i) ).split(" ");
                            int ind = 1;
                            while ( ind < Array.getLength( amounts ) ) {
                                if ( amounts[ind].equals("ONE") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.button1Val.amount() );
                                } else if ( amounts[ind].equals("TWO") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.button2Val.amount() );
                                } else if ( amounts[ind].equals("THREE") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.button3Val.amount() );
                                } else if ( amounts[ind].equals("FOUR") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.button4Val.amount() );
                                } else if ( amounts[ind].equals("POT") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.startUpApp.getPot().amount() );
                                }
                                ind++;
                            }
                            ...
                            ret = new String( retb );
                            if ( ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet().amount() >= theAIApp.getThisPlayer().getBankroll().amount() ) {
                                ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).setBet( 0.0f );
                                theAIApp.messageToServer( "BET ALLIN" );
                            ...
                            if ( theAIApp.startUpApp.getGame().getCurrBet().amount() > 0.0f ) {
                                if ( ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet().amount() <= theAIApp.startUpApp.getGame().getCurrBet().amount() ) {
                                    theAIApp.messageToServer("BET FOLD");
                                    theAIApp.log( "Tried to bet " + ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet() + ", but it wasn't enough so had to fold.", 3 );
                                    return new String( "FOLD" );
                                } else {
                                    theAIApp.messageToServer("BET CALL");
                                    theAIApp.log( "Tried to bet " + ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet() + ", but should have raised, so have to call.", 3 );
                                    ret = new String( "CALL" );
                                }
                            } else {
                                theAIApp.log( "Bet " + ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet(), 3 );
                            }    
                        }
                    }
                    if ( ((String)actionList.get(i)).startsWith( new String("RAISE") ) ) {
                        if ( !theAIApp.noLimit && !theAIApp.betLimit && !theAIApp.potLimit ) {
                            theAIApp.messageToServer( "BET ALLIN" );
                            ...
                            String amounts[] = ((String)actionList.get(i)).split(" ");
                            ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet(theAIApp.startUpApp.getGame().getCurrBet().amount() - ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getPrevBet().amount());
                            int ind = 1;
                            while ( ind < Array.getLength( amounts ) ) {
                                if ( amounts[ind].equals("ONE") ) {
                                    ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).addBet( theAIApp.button1Val.amount() );
                            ...
                            theAIApp.log( "Raise " + new PokerMoney( ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getBet().amount() - ( theAIApp.startUpApp.getGame().getCurrBet().amount() - ((Player)theAIApp.startUpApp.getPlayerList().get(pindex)).getPrevBet().amount() ) ), 3 );
                        }
                    }
                } 
            }
            if ( !actionPerformed ) {
                theAIApp.messageToServer("BET FOLD");
                theAIApp.log( "Warning : could not find an action for this AI to take, so had to fold.", 1 );
                return new String( "FOLD" );
            }
            theAIApp.messageToServer("BET SUBMIT");
            return ret;
