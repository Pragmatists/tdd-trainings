...
/****************************************************
 * The AIFrame class defines the window which is used to control and monitor the AIApp.
 *
 * @author Dan Puperi
 * @version 1.00
 *
 ...
 **/
    ...
    private AIApp              theAIApp;                        // The instance of the AIApp which created this AIFrame class
    private DefaultListModel   messageListModel;                // The listbox model describing how the message box works
    private JScrollPane        messageListScrollPane;           // The message area scroll bar definitions
    private JList              messageList;                     // The message area list of messages
    private int                messageNum;                      // Which message should be displayed at the top of the message list
    private DefaultListModel   dataListModel;                   // The listbox model describing how the data area works
    private JScrollPane        dataListScrollPane;              // The data area scroll bar definitions
    private JList              dataList;                        // The data area list of messages
    private JTextField         nameText;                        // The text field where the AI's name is entered
    ...
    private JComboBox          chooseType;                      // Combo box used to choose which type of AI to play with
    ...
    private String             playerType;                      // String describing what type of player this is - from the combo box
    private boolean            displayCheatData;                // Whether or not the cheat data should be displayed for this AI

/***********************
 * Constructor creates and instance of the AIFrame class
 *
 * @param title The title at the top of the window
 * @param a The AIApp instance to which this AIFrame belongs
 *
 **/
    public AIFrame( String title, AIApp a ) {
        ...
        dataListScrollPane.setVerticalScrollBarPolicy( JScrollPane.VERTICAL_SCROLLBAR_NEVER );
        dataListScrollPane.setHorizontalScrollBarPolicy( JScrollPane.HORIZONTAL_SCROLLBAR_NEVER );
...
/***********************
 * sendMessage() prints a message to the message box as the first element (on top)
 *
 * @param m The String message to print
 *
 **/
...
/***********************
 * updateMoneyLine() function is called to display updated bank and pot values on the window.  This version
 * updates with default values.
 **/
    public void updateMoneyLine() {
        updateMoneyLine( new PokerMoney(), new PokerMoney(), new PokerMoney(), new PokerMoney(), new String() ); 
    }

/***********************
 * updateMoneyLine() function is called to display updated bank and pot values on the window.  This version
 * the actual data is filled in.
 *
 * @param br The AI player's bankroll
 * @param pbet The AI player's bet
 * @param pot The total amount in the pot
 * @param bet The current high bet
 * @param name The name of the player with the current high bet.
 *
 **/
    public void updateMoneyLine( PokerMoney br, PokerMoney pbet, PokerMoney pot, PokerMoney bet, String name ) {
        theAIApp.log( "AIFrame.updateMoneyLine( " + br + ", " + pbet + ", " + pot + ", " + bet + ", " + name + " )", 3 );
    ...
    public void updateData() {
            ...
            if ( theAIApp.getAIAction().getAI() != null ) {
                ...
                if ( theAIApp.getAIAction().getAI().getGameClass() == null ) {
                    ...
                    if ( theAIApp.getAIAction().getAI().getGameClass().length() < 3 ) {
                        ...
                        gameName = theAIApp.getAIAction().getAI().getGameClass();
                    }
                }
        ...
        if ( myHand != null ) {
            ...
            if ( myHand.getNumHole() >= 1 ) {
                cardsToDisplay.append( myHand.getHoleCard( 0 ) + new String( "   " ) );
            }
            if ( myHand.getNumHole() >= 2 ) {
                cardsToDisplay.append( myHand.getHoleCard( 1 ) + new String( "   " ) );
            }
            if ( myHand.getNumHole() >= 3 ) {
                cardsToDisplay.append( myHand.getHoleCard( 2 ) + new String( "   " ) );
            }
            ...
            if ( theAIApp.startUpApp.getGame() != null ) {
                dataListModel.addElement( "My hand = " + HandEvaluator.nameHand( theAIApp.startUpApp.getGame().bestHand( myHand ) ) );
            }
            if ( theAIApp.startUpApp.getGame() != null ) {
                dataListModel.addElement( "Best hand = " + HandEvaluator.nameHand( theAIApp.startUpApp.getGame().getBestPossible() ) );
            }
        }
        for ( int i=0; i < 4; i++ ) {
...
/***********************
 * chooseAction class defines what happens when a player type choice is made via the combo box.
 **/
...
//----------------------
//    actionPerformed() function is called when the the user makes a choice from the combo box.
//
...
//----------------------
//    actionPerformed() function is called when the button is actually pressed.
//
        public void actionPerformed( java.awt.event.ActionEvent e ) {
...
/*********************** 
 * dataAction class defines what happens when the toggle cheat data button is pressed.
 **/
